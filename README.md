...
1.  **Основной файл `config.yaml`**: Здесь описываются метрики, глобальные настройки (таймауты, кеширование) и логирование.
2.  **Файл `.env`**: Для настроек, специфичных для окружения. Переменные из этого файла переопределяют значения из `config.yaml`. Создайте файл `.env` в корне проекта (можно скопировать из `.env.example`).
    *   `LISTEN_ADDRESS`: Адрес и порт для сервера (например, `0.0.0.0:9876`).
    *   `METRICS_PATH`: Путь для метрик (например, `/metrics`).
3.  **Внешний файл для черного списка**: Вы можете вынести список запрещенных команд в отдельный YAML-файл и указать путь к нему в переменной окружения `BLACKLIST_FILE_PATH` в файле `.env`.

### Структура `config.yaml`

*   `server`: Сетевые настройки. **Рекомендуется** настраивать через `.env`.
*   `logging`: Настройка логирования.
*   `global`: Глобальные настройки.
*   `metrics`: Список ваших метрик.
...
#### Проблема: Ошибка `duplicate metrics`

Представьте, что вы хотите посчитать количество сетевых соединений в каждом состоянии (`LISTEN`, `ESTAB`, и т.д.). Вы можете использовать команду, которая вернет несколько строк:
```sh
...
Чтобы решить эту проблему, нужно сделать каждую метрику уникальной. Динамические метки позволяют использовать часть вывода команды в качестве значения для метки (label).

В нашем примере мы можем использовать название состояния (`ESTAB`, `LISTEN`) как значение для метки `state`.

Вот как выглядит правильная конфигурация:
```yaml
metrics:
