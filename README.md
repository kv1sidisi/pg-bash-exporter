...
2.  Найдите актуальный релиз.
3.  Скачайте архив для вашей операционной системы (например, `pg-bash-exporter-linux-amd64.tar.gz`).
4.  Распакуйте архив:
    ```sh
...
*   `--config <путь/к/файлу.yaml>`: Указывает путь к конфигурационному файлу.
    ```sh
...
2.  Выполните команду:
    ...
    ```
    Эта команда соберет образ для экспортера и запустит все три сервиса в фоновом режиме.
